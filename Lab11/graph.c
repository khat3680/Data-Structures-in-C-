...
int has_vertex_graph(Graph *g, Vertex *v) {
	...
	//only if number of vertex is less than vertex count
...
//should not add duplicate vertex
...
//assign new vertex a sequential number & increment vertex count
...
int remove_vertex_graph(Graph *g, Vertex *v) {
	assert(g && v);
	if (is_null_graph(g) || !has_vertex_graph(g, v)) {
...
//	v->num = g->vertex_count;
//	g->vertex_count--;
//	g->vertex_list[g->vertex_count - 1] = NULL;
...
int has_edge_graph(Graph *g, Edge *e) {
	...
	//if 1 vertex doesn't exist, edge doesn't exist
	if (e->v1->num >= g->vertex_count || e->v2->num >= g->vertex_count) {
		return False;
	}

	//Graph doesn't contain directed or weighted edges
...
int add_edge_graph(Graph *g, Edge *e) {
	...
	//Add edge to AM, twice bc symmetrical
