...
Vertex* create_vertex(int n, char *s) {
	...
	//if user does not provide any label, set label as the int (which acts as serial num)
...
Edge* create_edge(Vertex *v1, Vertex *v2, int w, int d) {
	...
	//if both vertices are same, it is usually a loop, but not for this code
...
void destroy_edge(Edge **e) {
	...
	//destroying edge shouldn't destroy its vertices
	(*e)->v1 = NULL;
	(*e)->v2 = NULL;
	free(*e);
	*e = NULL;
...
int is_equal_edge(Edge *e1, Edge *e2) {
	...
	//if 1 edge weighted and other isn't, not equal

	if (e1->weight != e2->weight) {
		return False;
	}

	//if 1 edge directed and other isn't, not equal
	if (e1->directed != e2->directed) {
		return False;
	}

	//reaching here means both edges are equal in weight and directed

	//if vertices match, is equal
	if (is_equal_vertex(e1->v1, e2->v1) & is_equal_vertex(e1->v2, e2->v2)) {
		return True;
	}

	//order of vertices in undirected graph not important
