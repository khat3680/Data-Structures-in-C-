 ...
 * Student email:khat36680@mylaurier.ca
 * -------------------------
 */
 ...
 * Average Case:	O(n^2)			O(n)
 *
 * Memory:			O(1) // inplace does not copy whole array
 * Notes:	Selection is better than bubble in swaps, but comparisons are more.
 * ----------------------------------------------------
 */
void selection_sort(Data *array, const int size) {
	...
	for(i=0;i<size -1;i++){
		min_index =i;
		for(j = i + 1;j < size;j++) // finds the minimum value.
			if(compare_data(&array[j],&array[min_index]) == 2)
				min_index = j;

		swap_data(&array[min_index],&array[i]);
	...
	int pivot_index = start;
		for(int j = start;j< end;j++)
			if(compare_data(&array[j],&pivot) == 2) {
				swap_data(&array[pivot_index],&array[j]);
				pivot_index++;
			}
		swap_data(&array[pivot_index],&array[end]);
