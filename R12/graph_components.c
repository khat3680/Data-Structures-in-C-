...
void destroy_edge(Edge **e) {
	...
	// destroying an edge should not destroy is veertices;
	(*e)->v1 = NULL;
	(*e)->v2 = NULL;
	free(*e);
	*e = NULL;
...
int is_equal_edge(Edge *e1, Edge *e2) {
	...
	// if one edge is directed and the other is not --> not equal;
	if (e1->directed != e2->directed)
		return False;

	//Reaching here means both edges are equal in terms of weight an directed

	//if vertices match -->equal
	if(is_equal_vertex(e1->v1,e2->v2) && is_equal_vertex(e1->v2,e2->v2))
		return True;

	if(is_equal_vertex(e1->v2,e2->v1) && is_equal_vertex(e1->v1,e2->v1))
		return True;
